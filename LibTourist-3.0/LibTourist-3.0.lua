...
local currentPlayer = nil -- Будет установлено при первом событии

local function UpdatePlayerInfo()
    local name = UnitName("player")
    local guid = UnitGUID("player")
    if name and guid then
        currentPlayer = name .. guid
    elseif name then
        currentPlayer = name -- fallback только на имя
    end
    
    local _, race = UnitRace("player")
    isHorde = (race == "Orc" or race == "Troll" or race == "Tauren" or race == "Scourge" or race == "BloodElf")
...
local MAX_LEVEL = 80 -- Для WotLK максимальный уровень 80
...
local function PLAYER_LEVEL_UP(self, level)
    ...
    -- Обновляем фракцию персонажа (на случай смены персонажа)
    local _,race = UnitRace("player")
    isHorde = (race == "Orc" or race == "Troll" or race == "Tauren" or race == "Scourge" or race == "BloodElf")
    ...
    for zone in pairs(lows) do
        if zone then
            -- Пропускаем транспорты
            if types[zone] ~= "Transport" then
                -- Получаем уровни зоны
                local low, high = 0, 0
                local success, l, h = pcall(self.GetLevel, self, zone)
                if success and type(l) == "number" and type(h) == "number" then
                    low, high = l, h
                end
                
                local zoneType = types[zone] or "Zone"
                
                -- Обычные зоны и PvP зоны
                if zoneType == "Zone" or zoneType == "PvP Zone" or zoneType == "City" then
                    if type(low) == "number" and type(high) == "number" and low > 0 and high > 0 then
                        if type(playerLevel) == "number" and low <= playerLevel and playerLevel <= high then
                            recZones[zone] = true
                        end
                    end
                end
                ...
                -- Поля боя
                if zoneType == "Battleground" then
                    if type(low) == "number" and low > 0 then
                        local pLevel = playerLevel
                        -- Спецобработка для некоторых полей боя
                        if zone == BZ["Alterac Valley"] then
                            pLevel = pLevel - 1
                        end
                        if type(pLevel) == "number" and pLevel >= low then
                            -- Для полей боя проверяем также брекеты уровней
                            if pLevel >= MAX_LEVEL or (type(pLevel) == "number" and math.fmod(pLevel, 10) >= 6) then
                                recInstances[zone] = true
                            end
                        end
                    end
...
function Tourist:GetLevel(zone)
    ...
    if types[zone] == "Battleground" then
        -- Убеждаемся что playerLevel - число
        local pLevel = type(playerLevel) == "number" and playerLevel or 1
        
        -- Warsong Gulch (10-80)
        if zone == BZ["Warsong Gulch"] then
            if pLevel >= 71 then
                return 71, 80
            elseif pLevel >= 61 then
                return 61, 70
            elseif pLevel >= 51 then
                return 51, 60
            elseif pLevel >= 41 then
                return 41, 50
            elseif pLevel >= 31 then
                return 31, 40
            elseif pLevel >= 21 then
                return 21, 30
            elseif pLevel >= 11 then
                return 11, 20
            else
                return 10, 10
            end
        ...
        elseif zone == BZ["Strand of the Ancients"] then
            if pLevel >= 71 then
                return 71, 80
            else
                return 0, 0
    ...
    -- Для обычных зон и инстансов
    local low = lows[zone]
    local high = highs[zone]
    
    -- Проверяем тип и приводим к числам
...
function Tourist:GetLevelColor(zone)
    ...
    if types[zone] == "Battleground" then
        -- Проверяем доступность поля боя
        local low, high = self:GetLevel(zone)
        
        -- Проверяем что low и high - числа
        if type(low) ~= "number" then low = 0 end
        if type(high) ~= "number" then high = 0 end
        
        if low == 0 and high == 0 then
            return 0.5, 0.5, 0.5 -- Серый - недоступно
        end
        
        -- Красный - слишком низкий уровень
        if type(pLevel) == "number" and type(low) == "number" and pLevel < low then
            return 1, 0, 0
        end
        
        -- Желтый - максимальный уровень
        if pLevel >= MAX_LEVEL then
            return 1, 1, 0
        end
        
        -- Градиент в зависимости от уровня
        local bgLevel = pLevel
        if zone == BZ["Alterac Valley"] or zone == BZ["Eye of the Storm"] or zone == BZ["Strand of the Ancients"] then
            bgLevel = bgLevel - 1
        end
        
        bgLevel = math.fmod(bgLevel, 10)
        if bgLevel <= 5 then
            return 1, bgLevel / 10, 0
        elseif bgLevel <= 7 then
            return 1, (bgLevel - 3) / 4, 0
        else
            return (9 - bgLevel) / 2, 1, 0
        end
    end
    ...
    -- Город или нет данных об уровнях
    if low <= 0 and high <= 0 then
        return 1, 1, 1 -- Белый
    end
    
    -- Проверяем что pLevel - число
    if type(pLevel) ~= "number" then
        pLevel = 1
    end
    
    -- Зона текущего уровня
    if pLevel == low and pLevel == high then
        return 1, 1, 0 -- Желтый
    end
    
    -- Слишком низкий уровень (красный)
    if type(low) == "number" and pLevel <= low - 3 then
        return 1, 0, 0
    end
    
    -- Ниже рекомендуемого уровня (оранжевый)
    if type(low) == "number" and pLevel <= low then
        return 1, (pLevel - low - 3) / -6, 0
    end
    
    -- В диапазоне рекомендуемых уровней (желто-зеленый градиент)
    if type(low) == "number" and type(high) == "number" and pLevel <= (low + high) / 2 then
        return 1, (pLevel - low) / (high - low) + 0.5, 0
    end
    
    -- Выше среднего уровня (зеленый)
    if type(low) == "number" and type(high) == "number" and pLevel <= high then
        return 2 * (pLevel - high) / (low - high), 1, 0
    end
    
    -- Немного выше максимального уровня (голубой)
    if type(high) == "number" and pLevel <= high + 3 then
        local num = (pLevel - high) / 6
        return num, 1 - num, num
    end
    
    -- Слишком высокий уровень (серый)
    return 0.5, 0.5, 0.5
end
...
setmetatable(cost, {
    __index = function(self, vertex)
        if not vertex or not lows[vertex] then  -- Исправлено: проверяем vertex
            return 1
        end
...
function Tourist:IterateBorderZones(zone, zonesOnly)
	...
	elseif type(path) == "table" then
		...
		-- ИСПРАВЛЕНО: проверяем что path существует и не nil
		if zonesOnly and path and not yardYOffsets[path] then
			return retNil
		end
		return retOne, path
	end
...
do
	...
	Tourist.frame:RegisterEvent("PLAYER_LOGIN")
	Tourist.frame:RegisterEvent("PLAYER_LOGOUT")
	Tourist.frame:SetScript("OnEvent", function(frame, event, ...)
		if event == "PLAYER_LOGIN" or event == "PLAYER_ENTERING_WORLD" or event == "PLAYER_LEVEL_UP" then
			local oldPlayer = currentPlayer
			UpdatePlayerInfo()
			
			-- Если персонаж сменился
			if oldPlayer and currentPlayer and oldPlayer ~= currentPlayer then
				for k in pairs(recZones) do recZones[k] = nil end
				for k in pairs(recInstances) do recInstances[k] = nil end
				for k in pairs(cost) do cost[k] = nil end
			end
			
			if event ~= "PLAYER_LEVEL_UP" then
				PLAYER_LEVEL_UP(Tourist, UnitLevel("player"))
			else
	...
	-- Принудительно устанавливаем уровни для полей боя (WotLK)
	local bgLevels = {
		[BZ["Warsong Gulch"]] = {10, 80},
		[BZ["Arathi Basin"]] = {20, 80},
		[BZ["Alterac Valley"]] = {51, 80},
		[BZ["Eye of the Storm"]] = {61, 80},
		[BZ["Strand of the Ancients"]] = {71, 80},
	}

	for bg, levels in pairs(bgLevels) do
		if zones[bg] then
			zones[bg].low = levels[1]
			zones[bg].high = levels[2]
		end
	end
	...
	-- Исправлено для WoW 3.3.5 (WotLK)
	local continentNames = { GetMapContinents() }

	-- В WoW 3.3.5 GetMapContinents() возвращает только Калимдор и Восточные Королевства
	-- Добавляем Outland и Northrend вручную
	if #continentNames == 2 then
		tinsert(continentNames, "Запределье")  -- Outland
		tinsert(continentNames, "Нордскол")    -- Northrend
	end

	-- Таблица соответствия локализованных названий континентов английским
	local continentMapping = {
		-- Русская локализация
		["Калимдор"] = "Kalimdor",
		["Восточные королевства"] = "Eastern Kingdoms",
		["Запределье"] = "Outland",
		["Нордскол"] = "Northrend",
		-- Английские названия (на всякий случай)
		["Kalimdor"] = "Kalimdor",
	...
	for continentID, continentName in ipairs(continentNames) do
		-- Получаем английское название континента
		local continentEnglish = continentMapping[continentName] or continentName
		
		-- Безопасное переключение карты
		local success = false
		
		if continentID <= 2 then
			-- Для Калимдора (1) и Восточных Королевств (2)
			SetMapZoom(continentID)
			success = true
		elseif continentName == "Запределье" or continentName == "Outland" or continentEnglish == "Outland" then
			-- Для Outland
			pcall(function() SetMapZoom(0, 3) end)
			pcall(function() SetMapZoom(3) end)
			success = true
		elseif continentName == "Нордскол" or continentName == "Northrend" or continentEnglish == "Northrend" then
			-- Для Northrend
			pcall(function() SetMapZoom(0, 4) end)
			pcall(function() SetMapZoom(4) end)
			success = true
		end
		
		-- Получаем текстуру континента
		if success and zones[continentEnglish] then
			local mapInfo = GetMapInfo()
			if mapInfo then
				zones[continentEnglish].texture = mapInfo
				-- Также обновляем локализованное название, если оно есть
				if zones[continentName] then
					zones[continentName].texture = mapInfo
				end
			end
		end
		
		-- Получаем зоны континента
		local mapID = continentID
		if continentEnglish == "Outland" then
			mapID = 3
		elseif continentEnglish == "Northrend" then
			mapID = 4
		end
		
		-- Пропускаем, если у континента нет размеров
		if success and zones[continentEnglish] and zones[continentEnglish].yards then
			local zoneNames = {}
			pcall(function() zoneNames = { GetMapZones(mapID) } end)
			
			if #zoneNames > 0 then
				local continentYards = zones[continentEnglish].yards
				...
				for _ = 1, #zoneNames do
					...
					-- Пропускаем континенты (они уже обработаны)
					if continentMapping[name] then
						break
					end
					...
					-- Также обновляем по английскому названию, если оно отличается
					local englishName = BZR[name]
					if englishName and englishName ~= name and zones[englishName] then
						zones[englishName].yards = texX * continentYards
						zones[englishName].x_offset = scrollX * continentYards
						zones[englishName].y_offset = scrollY * continentYards * 2/3
						zones[englishName].texture = fileName
					end
				end
			end
		end
	...
	-- Убеждаемся, что у всех континентов есть текстуры
	for localizedName, englishName in pairs(continentMapping) do
		if zones[englishName] and not zones[englishName].texture then
			if englishName == "Kalimdor" then
				zones[englishName].texture = "Kalimdor"
			elseif englishName == "Eastern Kingdoms" then
				zones[englishName].texture = "EasternKingdoms"
			elseif englishName == "Outland" then
				zones[englishName].texture = "Outland"
			elseif englishName == "Northrend" then
				zones[englishName].texture = "Northrend"
			end
		end
		
		-- Копируем текстуру на локализованное название, если оно есть
		if zones[localizedName] and zones[englishName] and zones[englishName].texture then
			zones[localizedName].texture = zones[englishName].texture
			zones[localizedName].yards = zones[englishName].yards
			zones[localizedName].x_offset = zones[englishName].x_offset
			zones[localizedName].y_offset = zones[englishName].y_offset
		end
	end

	SetMapToCurrentZone()

	-- Сохраняем копию таблицы zones перед её удалением
	local finalZones = zones

	-- Очищаем original zones
	zones = nil

	-- Теперь используем finalZones
	for k,v in pairs(finalZones) do
		if k and v then
			lows[k] = tonumber(v.low) or 0
			highs[k] = tonumber(v.high) or 0
